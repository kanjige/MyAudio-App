...
public class AudioBook extends AudioContent
{
  ...
  // Play the audiobook by setting the audioFile to the current chapter title (from chapterTitles array list) 
	...
	public void play()
	{
		setAudioFile(chapterTitles.get(currentChapter));	//Sets current chapter of audiobook, then plays said chapter
		super.play();
	...
	{
		for (int i = 0; i < chapterTitles.size(); i++){		//Iterates through chapters of audiobook, prints them
	...
	{
		if (chapter >= 1 && chapter <= chapters.size())		//Checks if chapter is within range, if so, sets current chapter as so
		{
	...
	{
		AudioBook otheraudbook = (AudioBook) other;
		if (super.equals(otheraudbook) == true && this.getAuthor().equals(otheraudbook.getAuthor()) && this.getNarrator().equals(otheraudbook.getNarrator())){
			return true;
		}
		return false;
	}
